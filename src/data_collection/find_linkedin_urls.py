    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"      Ã¢Å¡Â  Rate limited, waiting 60s...")
                time.sleep(60)
                continue
            
            elif response.status_code == 403:
                print(f"      Ã¢Å¡Â  API quota exceeded")
                return {
...
def find_linkedin_urls_batch(queries_df, batch_name, delay=DELAY_BETWEEN_REQUESTS, 
    ...
    print(f"\n[Ã¢â€ â€™] Processing batch: {batch_name}")
    ...
    print(f"\n    Ã¢Å“â€œ Completed: {found_count}/{total} URLs found ({100*found_count/total:.1f}%)")
    if found_count > 0:
        print(f"    Ã¢Å“â€œ Verified: {verified_count}/{found_count} ({100*verified_count/found_count:.1f}%)")
...
def process_batch_file(batch_num, resume=True):
    ...
    if not os.path.exists(batch_file):
        print(f"ÃƒÂ¢Ã‚ÂÃ…â€™ Batch file not found: {batch_file}")
        return None
...
def combine_all_results():
    ...
    print(f"\nÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Combined {len(all_files)} files ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ {output_path}")
    ...
    for batch_file in batch_files:
        ...
        if batch_num in completed:
            ...
            if os.path.exists(results_file):
                ...
                status = f"ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ {found}/{queries} found, {verified} verified"
            else:
                status = "ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Complete"
        else:
            checkpoint = load_checkpoint(batch_num)
            status = f"ÃƒÂ¢Ã‚ÂÃ‚Â¸ {checkpoint['queries_processed']}/{queries} done" if checkpoint else "ÃƒÂ¢Ã¢â‚¬â€Ã¢â‚¬Â¹ Not started"
...
def run_verification_cmd(apply_filter=False):
    ...
    if not os.path.exists(combined_file):
        print(f"\nÃƒÂ¢Ã‚ÂÃ…â€™ File not found: {combined_file}")
    ...
    print(f"\nÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Updated with verification columns: {combined_file}")
    ...
    if apply_filter:
        ...
        print(f"ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Saved filtered data to: {verified_file}")
...
def run_prototype(company_name):
    ...
    print("=" * 60)
    print("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â¬ PROTOTYPE MODE")
    print("=" * 60)
    ...
    if not os.path.exists(SP500_DATA_PATH):
        print(f"\nÃƒÂ¢Ã‚ÂÃ…â€™ Data not found: {SP500_DATA_PATH}")
        return
    ...
    if not matches:
        print(f"\nÃƒÂ¢Ã‚ÂÃ…â€™ No match for '{company_name}'")
        return
    ...
    print(f"\nÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Selected: {selected}")
    ...
    for _, row in company_df.iterrows():
        print(f"  ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢ {row['director_name_clean']}")
    ...
    for _, row in company_df.iterrows():
        if pd.notna(row.get('linkedin_url')):
            v = "ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“" if row.get('verified') else "ÃƒÂ¢Ã…Â¡Ã‚Â "
            print(f"  {v} {row['director_name_clean']}")
            print(f"    ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ {row['linkedin_url']}")
        else:
            print(f"  ÃƒÂ¢Ã…â€œÃ¢â‚¬â€ {row['director_name_clean']}")
    ...
    print(f"\nÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Done! {found}/{len(company_df)} found, {verified} verified")
...
Examples:
  ...
  python3 find_linkedin_urls_sp500.py --verify --apply
    ...
    if args.status:
        ...
        if not os.path.exists(QUERIES_PATH):
            print(f"ÃƒÂ¢Ã‚ÂÃ…â€™ Run prepare_linkedin_queries_sp500.py first.")
            return
        ...
        if args.batch.lower() == 'all':
            for i in range(1, len(batch_files) + 1):
                ...
                if 'quota_exceeded' in result.get('search_status', pd.Series()).values:
                    print("\nÃƒÂ¢Ã…Â¡Ã‚Â  Quota exceeded. Continue tomorrow.")
                    break
