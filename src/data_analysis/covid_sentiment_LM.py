#!/usr/bin/env python3
"""
Analyze COVID-19 Sentiment in Director Posts (Sanity Check)
===========================================================
Sentiment analysis using Loughran-McDonald financial dictionary for COVID-related posts.
This serves as a sanity check - we expect NEGATIVE sentiment for pandemic-related posts.

Usage:
    python3 covid_sentiment_LM.py
    python3 covid_sentiment_LM.py posts_with_metadata_20260203_125602.csv
"""
...
COVID_KEYWORDS = [
    ...
    'ppe', 'mask', 'ventilator', 'flatten the curve', 'shelter in place',
    'essential worker', 'frontline', 'virtual meeting', 'zoom fatigue'
]
...
def is_covid_related(text):
    """Check if text contains COVID-related keywords (using word boundaries)"""
    if pd.isna(text):
        return False
    ...
    for kw in COVID_KEYWORDS:
        # For keywords with spaces or hyphens, use exact matching
        if ' ' in kw or '-' in kw:
            if kw in text_lower:
                return True
        else:
...
def main():
    ...
    parser.add_argument('--all-posts', action='store_true', help='Analyze all posts, not just COVID-related')
    args = parser.parse_args()
    
    print("=" * 80)
    print("  LOUGHRAN-MCDONALD SENTIMENT ANALYSIS")
    print("  COVID-19 Sentiment in S&P 500 Director Posts")
    print("=" * 80)
    ...
    # Filter to COVID-related posts unless --all-posts flag is set
    if not args.all_posts:
        print("\nFiltering to COVID-related posts...")
        df['is_covid_related'] = df['post_text'].apply(is_covid_related)
        df_analysis = df[df['is_covid_related']].copy()
        print(f" Found {len(df_analysis):,} COVID-related posts ({len(df_analysis)/len(df)*100:.2f}%)")
    else:
    ...
    post_type = "all_posts" if args.all_posts else "covid_posts"
